(ns amex-auth.main
  "Kotodama WASM entrypoint for the Amex_auth clean-room actor (L4) — Clojure port.
  ...
  facts (`amex_auth.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "amex_auth")
(def tier "L4")
...
(def entity-specs
  [{:entity "Customer"       :plural "customers"       :id-prefix "amexauth_cus"
    :fields [:email :name :phone :balance]             :required [:email :name]
    :coerce {:balance :int}                :refs {}}
   {:entity "PaymentIntent"  :plural "paymentintents"  :id-prefix "amexauth_pay"
    :fields [:customerId :amount :currency :status]    :required [:amount :currency]
    :coerce {:amount :int}                 :refs {:customerId "Customer"}}
   {:entity "Charge"         :plural "charges"         :id-prefix "amexauth_cha"
    :fields [:customerId :paymentIntentId :amount :currency :status] :required [:amount :currency]
    :coerce {:amount :int}                 :refs {:customerId "Customer" :paymentIntentId "PaymentIntent"}}
   {:entity "Refund"         :plural "refunds"         :id-prefix "amexauth_ref"
    :fields [:chargeId :amount :reason :status]        :required [:amount :reason]
    :coerce {:amount :int}                 :refs {:chargeId "Charge"}}
   {:entity "Payout"         :plural "payouts"         :id-prefix "amexauth_pay"
    :fields [:amount :currency :arrivalDate :status]   :required [:amount :currency]
    :coerce {:amount :int}                 :refs {}}
   {:entity "PaymentMethod"  :plural "paymentmethods"  :id-prefix "amexauth_pay"
    :fields [:customerId :type :last4 :brand]          :required [:type :last4]
    :coerce {}                             :refs {:customerId "Customer"}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"amex_auth.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "amex_auth-compat" :tier tier :entities entities} 200])
...
(defn start! [] :amex_auth-compat/ready)
